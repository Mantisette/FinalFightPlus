...
var map_size = Vector2(global.MAP_WIDTH, global.MAP_HEIGHT)
var cellmap = global.map
...
func find_path(world_start, world_end):
  self.path_start_position = world_start # world_to_map(world_start)
  self.path_end_position = world_end # world_to_map(world_end)
  _recalculate_path()
  var path_world = []
  for point in _point_path:
    var point_world = global.tile_to_pixel_center(point) # map_to_world(Vector2(point.x, point.y)) + _half_cell_size
...
#  set_cell(point_end.x, point_end.y, 2)
  ...
  var last_point = global.tile_to_pixel_center(point_start) # map_to_world(Vector2(point_start.x, point_start.y)) + _half_cell_size
  for index in range(1, len(_point_path)):
    var current_point = global.tile_to_pixel_center(_point_path[index]) # map_to_world(Vector2(_point_path[index].x, _point_path[index].y)) + _half_cell_size
...
func _set_path_start_position(value):
  if cellmap[value.x][value.y] == 1: # if value in obstacles:
    return
